...
type grid = bool list * int

let make_grid_opt width height =
  match (width, height) with
  | width, height when width <= 0 || height <= 0 -> None
  | width, height -> Some (List.init (width * height) (fun _ -> false), width)
...
let grid_get_opt (grid, width) x y =
  match (x, y) with
  | x, y when x < 0 || x > width || y < 0 || y > List.length grid / width -> None
  | x, y -> Some (List.nth grid (grid_index (grid, width) x y))

let grid_get grid x y = grid_get_opt grid x y |> Option.get

let grid_set (grid, width) x y value =
  match (x, y) with
  | x, y when x < 0 || x > width || y < 0 || y > List.length grid / width -> (grid, width)
  | x, y -> (List.mapi (fun index v -> if index == (y * width) + x then value else v) grid, width)
...
let grid_count (grid, _) = List.fold_left (fun acc curr -> if curr then acc + 1 else acc) 0 grid
