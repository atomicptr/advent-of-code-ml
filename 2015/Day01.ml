...
let parse str = List.init (String.length str) (String.get str)
    |> List.map (fun s -> if s == '(' then Some Up else if s == ')' then Some Down else None)
    |> List.filter Option.is_some
    |> List.map Option.get

let eval_step lst value = match lst with
| [] -> value
| Up :: _ -> value + 1
| Down :: _ -> value - 1 

let rec eval lst value = match lst with
| [] -> value
| action :: rest -> eval rest @@ eval_step [action] value

let parse_and_eval str = eval (parse str) 0

let rec find_basement_entry_pos lst value pos = if value = (-1) then pos else match lst with
| [] -> (-1)
